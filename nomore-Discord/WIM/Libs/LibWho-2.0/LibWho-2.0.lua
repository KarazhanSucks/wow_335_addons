...
function lib:ProcessWhoResults()
	...
	local maxTriesToBreak = 10
	local lastValidIndex = 0
	...
	for i = 1, num do
		...
		if charname then
			...
			if (i - lastValidIndex) >= maxTriesToBreak then
				dbg("Breaking out after " .. maxTriesToBreak .. " consecutive nils (last valid at " .. lastValidIndex .. ", now at " .. i .. ")")
				break
			else
				dbg("GetWhoInfo returned nil for index " .. i)
			end
		end
	end
